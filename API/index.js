...
app.get("/video", async (req, res) => {
  ...
  try {
    ...
    page.on("request", (request) => {
      ...
      if (
        ...
        new URL(reqUrl).pathname.endsWith("/master.txt")
      ) {
        m3u8Url.push(reqUrl);
      }
      request.continue();
  ...
  console.log(m3u8Url);
  res.json({
    videoUrl: `https://anym3u8player.com/tv/video-player.php?url=${encodeURIComponent(
      m3u8Url
    )}`,
    proxied: `${m3u8Url}`,
    season,
...
app.get("/video-test", async (req, res) => {
  ...
  try {
    ...
    if (season == "01") {
      const targetUrl = `https://beta.awstream.net/watch?v=${name}-8211-episode-${episode}&lang=hin`;
      await page.goto(targetUrl, {
        waitUntil: "networkidle2",
        timeout: 30000,
      });
    } else {
      const targetUrl = `https://beta.awstream.net/watch?v=${name}-8211-season-${season}-8211-episode-${episode}&lang=hin`;
      await page.goto(targetUrl, {
        waitUntil: "networkidle2",
        timeout: 30000,
      });
